...
const languages: { [key: string]: Language } = {
  python: {
    name: "Python",
    extension: "py",
    template: `# TI Developer Community Registration
...
print("Registration complete!")`,
    parse: (code) => {
      ...
      lines.forEach(line => {
        if (line.includes('=') && !line.trim().startsWith('#') && !line.includes('print(')) {
...
const enrollment = "<enrollment_number>";
const course = "<your_course>";
...
console.log("Registration complete!");`,
    parse: (code) => {
      ...
      const lines = code.split('\n');
      lines.forEach(line => {
        if (line.includes('=') && !line.trim().startsWith('//') && !line.includes('console.log(')) {
          const [keyPart, value] = line.split('=').map(s => s.trim());
          const key = keyPart.replace(/^(const|let|var)\s+/, '');
          if (key && value) {
            data[key] = value.replace(/^["']|["'];$/g, '');
          }
        }
...
public class StudentRegistration {
  public static void main(String[] args) {
    ...
    String year = "<academic_year>";
    String enrollment = "<enrollment_number>";
    String course = "<your_course>";
  ...
  cpp: {
    name: "C++",
    extension: "cpp",
    template: `// TIT Developer Community Registration
    // Choos eyour desired Programming Language and code your details
// Replace the placeholder values with your information
#include <iostream>
#include <string>
int main() {
  std::string name = "<your_name>";
  std::string year = "<academic_year>";
  std::string enrollment = "<enrollment_number>";
  std::string course = "<your_course>";
  std::string college = "<college_name>";
  std::string email = "<your_email>";
  std::string phone = "<phone_number>";
...
const CodeEditorRegister = () => {
  ...
  const [code, setCode] = useState<string>(languages.python.template);
  ...
  // Initialize code with selected language
  useEffect(() => {
  ...
  const handleEditorChange: OnChange = (value,) => {
    ...
    const isFormComplete = Object.values(parsedData).every(value => !value.includes('<'));
    if (isFormComplete) {
  ...
  const isFormComplete = () => {
    return !Object.values(formData).some(value => value.includes('<'));
  };
  ...
  // Prevent editing of the final output line
  const handleEditorMount: OnMount = (editor) => {
    ...
    if (model) {
      model.onDidChangeContent(() => {
        ...
        const finalOutput = languages[language].template.split('\n').pop();
